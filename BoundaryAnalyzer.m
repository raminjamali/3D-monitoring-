function BoundaryAnalyzer
global Add Grain GrainProf Image MeanHeight Mean x1 x2 y1 y2 c L Height hLPlot hsub11 hLPlot11
hLPlot='axe';
%% Side Bar
...
%                     'HorizontalAlignment','center');
  ...
  Mean = uicontrol(hGui,'Style','text',...
                    'Position',SideBotDim+[0 7*50 0 0],...
                    'String',sprintf('Mean height:\n'),...
                    'HorizontalAlignment','center');
...
%%
  ...
  function Exclude_button(hObject,eventData)
    ...
    Height(k)=0;
    aq=find(Height>0);
    MeanHeight=sum(Height)./max(size(aq));
    set(Mean,'String',sprintf(['Mean height:\n',num2str(MeanHeight)]));
    subplot(hsub2);plot(L{k});hold on; plot(ones(size(L{k})).*max(L{k}),'r');plot(ones(size(L{k})).*min(L{k}),'r');hold off
    title('Height profile along the selected region')
  end
  function Bound_button(hObject,eventData)
    ...
    for k=1:length(c)
        ...
        Height(k)=max(L{k})-min(L{k});
    end
    Height=Height';
%     set(Min,'String',sprintf(['Minimum height:\n',num2str(min(Height))]));
    set(Mean,'String',sprintf(['Mean height:\n',num2str(mean(Height))]));
    k=1;
...
%     figure;mesh(h1);
  ...
  function save_button(hObject,eventData)
    ...
    if  strcmp(choicesave,'Save profile data')==1
        ...
        save([add,file],'Height','L','MeanOfHeight');
